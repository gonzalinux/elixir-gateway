defmodule ElixirGatewayWeb.Plugs.RateLimiter do
  ...
  def call(conn, _opts) do
    ...
    case check_rate_limits(user_bucket, ip_bucket, user_requests_per_minute, ip_requests_per_minute) do
      {:allow, user_count, ip_count} ->
        conn
        |> put_resp_header("x-ratelimit-user-limit", to_string(user_requests_per_minute))
        |> put_resp_header("x-ratelimit-user-remaining", to_string(user_requests_per_minute - user_count))
        |> put_resp_header("x-ratelimit-ip-limit", to_string(ip_requests_per_minute))
        |> put_resp_header("x-ratelimit-ip-remaining", to_string(ip_requests_per_minute - ip_count))
