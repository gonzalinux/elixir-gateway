defmodule ElixirGatewayWeb.Plugs.RequestForwarderTest do
  ...
  describe "request forwarding" do
    test "forwards GET request successfully", %{
      conn: conn,
      bypass: bypass,
    ...
    test "forwards query parameters correctly", %{
      conn: conn,
      bypass: bypass,
      target_url: target_url
    } do
  ...
  describe "header handling" do
    test "forwards allowed headers and excludes hop-by-hop headers", %{
      conn: conn,
      bypass: bypass,
      target_url: target_url
    } do
      Bypass.expect_once(bypass, "GET", "/api/test", fn conn ->
        ...
        refute Enum.any?(conn.req_headers, fn {name, _} ->
    ...
    test "forwards response headers while filtering hop-by-hop headers", %{
      conn: conn,
      bypass: bypass,
      target_url: target_url
    } do
  ...
  describe "error handling" do
    ...
    test "returns 502 when target service times out", %{conn: conn} do
      ...
      conn =
        conn
        # Unreachable
        |> assign(:target_url, "http://127.0.0.1:1")
        |> assign(:original_host, "api.example.com")
        |> Map.put(:request_path, "/api/test")
        |> Map.put(:query_string, "")
        |> RequestForwarder.call([])
  ...
  describe "request body handling" do
    test "handles binary/raw request bodies", %{
      conn: conn,
      bypass: bypass,
      target_url: target_url
    } do
      _binary_data = <<1, 2, 3, 4, 5>>

      Bypass.expect_once(bypass, "POST", "/api/upload", fn conn ->
        {:ok, _body, _} = Plug.Conn.read_body(conn)
      ...
      conn =
        ...
        |> Map.put(:query_string, "")
        |> Map.put(:method, "POST")
        # Empty to trigger raw body reading
        |> Map.put(:body_params, %{})
        |> RequestForwarder.call([])
  ...
  describe "URL building" do
    test "builds target URL with path and query string", %{
      conn: conn,
      bypass: bypass,
      target_url: target_url
    } do
